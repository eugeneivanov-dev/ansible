...
Prometheus and Alertmanager never listen on the network, Loki accepts log pushes from the lab subnet alone — Grafana is the stack's only user-facing endpoint. The alerting chain is closed from outside: an always-firing Watchdog alert pings an external service, so silence of the whole stack is itself an alert. Each role also checks its own listener after start: it reads the kernel's socket table and fails the play if the service is bound to any address other than the declared one, or if Alertmanager's gossip port is open at all. Pending restarts are flushed before the check, so it sees the restarted process. New service roles get the same check.
